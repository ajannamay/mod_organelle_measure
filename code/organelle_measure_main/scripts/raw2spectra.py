...
# folder_i=Path(f"{var.path_in}")
# folder_l=Path(f"{var.path_out}")
# folder_o=Path(f"{var.path_out}/spectra")
...
subfolders = var.raw2sp


current_folder='spectra'
...
df_meta.to_csv(f"{var.chx_im}/{current_folder}/meta_normalized.csv",index=False)
...
dict_files = {
    "blue": f"{var.path_xml}/Blue_color_peroxisome_vacuoles_choosen.xml",
    "red": f"{var.path_xml}/red_color_mito_lipid_choosenJuly22_1.xml"
}
dict_organelles = {
    "blue":["vacuole","peroxisome"],#["peroxisome","vacuole"]
    "red": ["mitochondria","LD"]
}
...
##
df_meta = pd.read_csv(f"{var.chx_im}/{current_folder}/meta_normalized.csv")
...
for folder,stem in zip(df_meta['folder'],df_meta['file']):
    ...
    file_label1 = f"{var.path_out}/{folder}/{var.output_folders['ilastk_segmentated']}/{dict_organelles[color][0]}_{file_raw.stem.partition('-')[2]}.tiff"
    
    print(file_label1)
    
    
    file_label2 = f"{var.path_out}/{folder}/{var.output_folders['ilastk_segmentated']}/{dict_organelles[color][1]}_{file_raw.stem.partition('-')[2]}.tiff"
    print(file_label2)
...
df_img.to_csv(f"{var.chx_im}/{current_folder}/spectra_images.csv",index=False)


# Plot spectra:

df_img = pd.read_csv(f"{var.chx_im}/{current_folder}/spectra_images.csv")
...
for color in ['blue','red']:
    ...
    for experiment in df_exp['experiment'].unique():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name = f"{experiment}:{dict_organelles[color][0]}",
                mode="lines+markers",
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name = f"{experiment}:{dict_organelles[color][1]}",
                mode="lines+markers",
    ...
    fig.write_html(f"{var.chx_im}/{current_folder}/white_unmix_comparison_{color}.html")