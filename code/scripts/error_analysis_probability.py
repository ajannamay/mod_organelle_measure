...
for subfolder in subfolders:
    for path_probs in (Path(f"{var.path_out}")/subfolder/f"{var.output_folders['ilastk_segmentated']}").glob("*.h5"):
        print(path_probs)
        organelle = ["peroxisome","vacuole"] if subfolder == "leucine-large-blue-gaussian" else path_probs.stem.partition('_')[0]
...
df_errors.to_csv(f"{var.chx_im}/spectra/image_error_probability_new.csv",index=False)
